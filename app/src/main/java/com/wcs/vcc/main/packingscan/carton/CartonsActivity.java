...
import com.wcs.wcs.databinding.ActivityCartonsBinding;
...
public class CartonsActivity extends EmdkActivity {
    ...
    private ActivityCartonsBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = DataBindingUtil.setContentView(this, R.layout.activity_cartons);
        ...
        adapter = new CartonAdapter(new RecyclerViewItemListener<Carton>() {
            ...
            public void onClick(Carton item, int position) {
                ...
                Intent intent = new Intent(CartonsActivity.this, PacksActivity.class);
            ...
            public void onLongClick(final Carton item, final int position) {
                AlertDialog dialog = new AlertDialog.Builder(CartonsActivity.this)
            ...
            public void onClickListen(final Carton item) {
                countSelected = 0;
                View view = LayoutInflater.from(CartonsActivity.this).inflate(R.layout.layout_update_package_type, null, false);

                Spinner spType = view.findViewById(R.id.sp_ppsc);

                ArrayAdapter<PackageType> adapter = new ArrayAdapter<>(CartonsActivity.this, android.R.layout.simple_list_item_1, packageTypes);
    ...
    public void onData(String data) {
        ...
        int cartonID = Integer.parseInt(data.substring(2,data.length()));
        int type = 0 ;
        for(int i =0; i< cartons.size();i++){
            if(cartons.get(i).DispatchingProductCartonID == cartonID){
                Intent intent = new Intent(CartonsActivity.this, PacksActivity.class);
                intent.putExtra("CARTON_ID", cartons.get(i).DispatchingProductCartonID);
                intent.putExtra("ORDER_NUMBER",  cartons.get(i).DispatchingOrderNumber);
                intent.putExtra("CARTON_NUMBER",  cartons.get(i).CartonNumber);
                intent.putExtra("COMPLETED",  cartons.get(i).Completed);
                intent.putExtra("ORDER_DATE", date);
                intent.putExtra("STORE_NUMBER",storeNumber);
                startActivity(intent);
                type=1;
        ...
        if(type==0)     scanCartonPickPackShipPack(data);
    }
    ...
    public  void onCheckMiss(View view){
        Intent intent = new Intent(CartonsActivity.this, CheckStatusPickPackShipActivity.class);
        intent.putExtra("STORE_NUMBER", storeNumber);
