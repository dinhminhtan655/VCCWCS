...
import com.wcs.vcc.api.EmployeeIDFindParameter;
import com.wcs.vcc.api.MyRetrofit;
import com.wcs.vcc.api.NoInternet;
import com.wcs.vcc.api.OverTimeDelUpdateParameter;
import com.wcs.vcc.api.OverTimeEntryParameter;
...
public class OverTimeEntryActivity extends BaseActivity implements AdapterView.OnItemSelectedListener {
    private final String TAG = OverTimeEntryActivity.class.getSimpleName();
    @BindView(R.id.ngoai_gio_acs_type)
    AppCompatSpinner acsType;
    @BindView(R.id.ngoai_gio_cb_many_date)
    CheckBox cbManyDate;
    @BindView(R.id.ngoai_gio_et_ma_nv)
    EditText etMaNV;
    @BindView(R.id.ngoai_gio_et_remark)
    EditText etRemark;
    @BindView(R.id.ngoai_gio_tv_ten_nv)
    TextView tvTenNV;
    @BindView(R.id.ngoai_gio_et_hours_qty)
    EditText etHoursQty;
    @BindView(R.id.ngoai_gio_tv_remain)
    TextView tvRemain;
    @BindView(R.id.ngoai_gio_tv_time_in)
    TextView tvTimeIn;
    @BindView(R.id.ngoai_gio_tv_time_out)
    TextView tvTimeOut;
    @BindView(R.id.bt_ngoai_gio_fromDate)
    Button btFromDate;
    @BindView(R.id.bt_ngoai_gio_toDate)
    Button btToDate;
    @BindView(R.id.ivArrowLeft)
    ImageView ivArrowLeft;
    @BindView(R.id.ivArrowRight)
    ImageView ivArrowRight;
    @BindView(R.id.ivArrowLeftTo)
    ImageView ivArrowLeftTo;
    @BindView(R.id.ivArrowRightTo)
    ImageView ivArrowRightTo;
    @BindView(R.id.tv_ngoai_gio_ot_month)
    TextView tvOTMonth;
    @BindView(R.id.tv_ngoai_gio_ot_year)
    TextView tvOTYear;
    @BindView(R.id.ngoai_gio_tv_total_time_work)
    TextView tvTotalTimeWork;
    private int employeeCode, storeId;
    ...
    private int c;
    private UUID employeeID, otSupperID;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent() != null && getIntent().hasExtra("FROM_DATE")) {
            ...
            isUpdate = true;
            employeeCode = getIntent().getIntExtra("EMPLOYEE_CODE", 0);
            otSupperID = UUID.fromString(getIntent().getStringExtra("OT_SUPPER_ID"));
            ...
            public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
                if (actionId == EditorInfo.IME_ACTION_DONE)
                    if (!etMaNV.getText().toString().equalsIgnoreCase("")) {
                        employeeCode = Integer.parseInt(etMaNV.getText().toString());
                        getEmployeeID(etMaNV);
                        return true;
                    }
                return false;
        ...
        if (isUpdate) {
            ...
            etMaNV.setText(String.format(Locale.US, "%d", employeeCode));
            getEmployeeID(etMaNV);
        }
    }
    ...
    public void getEmployeeID(final View view) {
        ...
        MyRetrofit.initRequest(this)
                .getEmployeeID(new EmployeeIDFindParameter(employeeCode, storeId, fromDate))
    ...
    public void overTimeOrderDetails() {
        if (tvTenNV.getText().length() > 0) {
            ...
            intent.putExtra("EMPLOYEE_ID", employeeID.toString());
