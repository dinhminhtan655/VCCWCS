package com.wcs.vcc.main.lichsuravao;
...
public class LichSuRaVaoAdapter extends ArrayAdapter<EmployeeInOutInfo> implements Filterable {
            ...
            protected FilterResults performFiltering(CharSequence constraint) {
                ...
                if (keyword.length() > 0) {
                    ...
                    for (int i = 0; i < dataOrigin.size(); i++) {
                        ...
                        if (name.contains(keyword) || Integer.toString(info.EmployeeCode).contains(keyword))
    ...
    static class ViewHolder {
        @BindView(R.id.item_tv_in_out_id)
        TextView tvID;
        @BindView(R.id.item_tv_in_out_name)
        TextView tvName;
        @BindView(R.id.item_tv_in_out_position)
        TextView tvPosition;
        @BindView(R.id.item_tv_in_out_department)
        TextView tvDepartment;
        @BindView(R.id.item_tv_in_out_night_shift)
        TextView tvNight;
        @BindView(R.id.item_tv_in_out_time_work)
        TextView tvTimeWork;
        @BindView(R.id.item_tv_in_out_shift)
        TextView tvShift;
        @BindView(R.id.item_tv_in_out_status)
        TextView tvStatus;
        @BindView(R.id.item_tv_in_out_time_keeper)
        TextView tvTimeKeeper;
        @BindView(R.id.item_tv_in_out_remark)
        TextView tvRemark;
