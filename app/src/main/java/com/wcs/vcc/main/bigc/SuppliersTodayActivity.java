...
public class SuppliersTodayActivity extends BaseActivity implements View.OnClickListener,
        AdapterView.OnItemClickListener {

    public static final String PURCHASING_ORDER_ID = "PurchasingOrderID";
    public static final String HEADER = "Header";
    public static final String NEXT_ACTIVITY_HEADER = "NextActivityHeader";
    public static final int RC_BASKET_MOVEMENT = 9999;
    public static final String POSITION = "position";
    public static final String PURCHASING_ORDER_NUMBER = "PurchasingOrderNumber";

    private View.OnClickListener action;

    private SuppliersTodayAdapter adapter;
    private ListView lv;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        adapter = new SuppliersTodayAdapter(this, suppliers, callback);
    ...
    public void getSuppliersToday() {
        ...
        MyRetrofit.initRequest(this).getSuppliersToday(SuppliersTodayParameter.date(requestDate)).enqueue(new Callback<List<Supplier>>() {
            ...
            public void onFailure(Throwable t) {
                dismissDialog(dialog);
                RetrofitError.errorWithAction(SuppliersTodayActivity.this, t, TAG, snackBarView, action);
            }
        });
    }
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        ...
        if (supplier != null) {
            ...
            Intent intent = new Intent(this, StoresOfSupplierActivity.class);
            intent.putExtra(PURCHASING_ORDER_ID, purchasingOrderId);
            intent.putExtra(PURCHASING_ORDER_NUMBER, supplier.getPurchasingOrderNumber());

            intent.putExtra(HEADER, String.format("%s > %s", btChooseDate.getText().toString(), supplier.getNameShort()));
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == RC_BASKET_MOVEMENT && resultCode == RESULT_OK) {
            String bmNumber = data.getStringExtra(BasketMovementActivity.BM_NUMBER);
            int position = data.getIntExtra(SuppliersTodayActivity.POSITION, 0);
            Supplier supplier = suppliers.get(position);
            if (supplier != null) {
                supplier.setBasketMovementNumber(bmNumber);
            }
            adapter.notifyDataSetChanged();
        }
    }
    ...
    private BasketMovementReturnCallback callback = new BasketMovementReturnCallback() {
        @Override
        public void onClick(String bmNumber) {
            insertBasketMovementReturn(bmNumber);
        }
    };

    public void insertBasketMovementReturn(String bmNumber) {
        final ProgressDialog dialog = Utilities.getProgressDialog(this, "Đang trả khay...");
        dialog.show();
        if (!WifiHelper.isConnected(this)) {
            dismissDialog(dialog);
        ...
        MyRetrofit.initRequest(this).insertBasketMovementReturn(new InsertBasketMovementReturnParameter(bmNumber, username)).enqueue(new Callback<String>() {
            ...
            public void onFailure(Throwable t) {
                dismissDialog(dialog);
                RetrofitError.errorNoAction(SuppliersTodayActivity.this, t, TAG, snackBarView);
            }
        });
    }


    interface BasketMovementReturnCallback {
        void onClick(String basketMovementNumber);
    }
}
